...
if(isset($_POST['update'])){
    ...
    //validate the data
        if(empty($title) || empty($property_type) || empty($listing_purpose) || empty($payment_flexibility) || empty($description) || empty($price) || empty($full_address) || empty($lga) || empty($state) || empty($bedrooms) || empty($bathrooms) || empty($toilets) || empty($furnishing) || $property_id <= 0){
            $_SESSION['errormsg'] = "All fields are required. Kindly fill the form completely";
            header("location:../agent/agent_dashboard_listings.php");
            exit;
        } 
   ...
   // $agentdetails = $agent->fetch_agent_details($email);
    ...
    $response = $agent->update_listing($property_type, $title, $description, $price, $payment_flexibility, $full_address, $lga, $state, $bedrooms, $bathrooms, $toilets, $furnishing, $parking_space, $electricity_supply, $water_supply, $security, $pop_ceiling, $tiled_floor, $prepaid_meter,  $unique_filename, $property_id);

    //$testing = new Agent();
    //$res = $testing->insert_property_media($_SESSION['agent_id'], $unique_filename);

    if($response){
        $_SESSION['successmsg'] = "Listing Updated successfully";
